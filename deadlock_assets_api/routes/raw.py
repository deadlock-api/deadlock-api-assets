...
@router.get("/heroes")
def get_raw_heroes(client_version: ValidClientVersions | None = None) -> FileResponse:
...
@router.get("/items")
def get_raw_items(client_version: ValidClientVersions | None = None) -> FileResponse:
    if client_version is None:
        client_version = ValidClientVersions(LATEST_VERSION)
